module best
	using cst, kunsat, param, BlackBoxOptim
	...
	function BESTG(Time, θ_Ini, θs, θr, Ks, RingRadius, Sorptivity, Kr_θini; Flag_Best="Best_Gi")
		...
		function INFILTRATION_3D_TRANSIT(Time, Sorptivity, A, B, Ks)
			return Inf = Sorptivity * (Time^0.5) + (A * (Sorptivity^2.) + B*Ks)*Time
		end

		function INFILTRATION_3D_STEADY_BESTG(Time, Sorptivity, A, B, Ks)
			return Inf = (A*Sorptivity^2. + Ks)*Time + best.C(B) * (Sorptivity^2.) / Ks
		end

		function  INFILTRATION_3D_STEADY_BESTGI(Time, Time_TransStead, Sorptivity, A, B, Ks)
			return Inf = (A*Sorptivity^2. + Ks) * (Time - Time_TransStead) + INFILTRATION_3D_TRANSIT(Time_TransStead, Sorptivity, A, B, Ks)
		end
		...
		if Time <= Time_TransStead 
			Inf = INFILTRATION_3D_TRANSIT(Time, Sorptivity, A, B, Ks)
		else
			if Flag_Best == "Best_G"
				Inf = INFILTRATION_3D_STEADY_BESTG(Time, Sorptivity, A, B, Ks)
			else
				Inf = INFILTRATION_3D_STEADY_BESTGI(Time, Time_TransStead, Sorptivity, A, B, Ks)
			end
		end
		return Inf
	end
	...
	function A(θs, θ_Ini, RingRadius)
		A = cst.γ / ( RingRadius * (θs-θ_Ini) ) # Units (mm-1)
		return A
	end
	...
	function INFILTRATION_1D_TRANSIT(Time, Sorptivity, B, Ks)
		Inf = Sorptivity * (Time^0.5) + B*Ks*Time
		return Inf
	end
	...
	function  INFILTRATION_1D_STEADY(Time, Sorptivity, A, B, Ks)
		Inf = Ks * Time + best.C(B) * (Sorptivity^2.) / Ks
		return Inf
	end
    ...
    #= =============== KOSUGI =============== =#
	module kg
		using cst, kunsat, param, BlackBoxOptim, sorptivity, wrc, best, thetaTime, stats,  Optim, array, quasiExact, relationship
		export BEST_INVERSE_SORPTIVITY, BEST_INVERSE, BEST_INVERSE_IMPROVED, BESTG_INVERSE, BESTG_INVERSE_SORPTIVITY,  STEADY_TRANSIT_BESTG,  BESTG_INVERSE_σmodel
		...
		function BESTG_INVERSE(Time, iT_N, Inf_3D_Obs, θ_Ini, Se_Ini, θs, RingRadius, iT_TransStead, Time_TransStead; Option_Opt_σ=true, Flag_Best="Best_Gi", σ=1.)
			...
			if Option_Opt_σ
				Optimization = BlackBoxOptim.bboptimize(X -> OF_BEST(Time[1:iT_N], Inf_3D_Obs[1:iT_N], iT_N, θ_Ini, θs, θr, 10.^X[1], X[2], 10.^X[3], RingRadius,iT_TransStead, Time_TransStead, Flag_Best, Se_Ini, P_Stead, P_Trans); SearchRange = [(log10(param.Hkg_Min), log10(param.Hkg_Max)), (param.σ_Min, param.σ_Max), (param.Ks_Log_Min, param.Ks_Log_Max)], NumDimensions=3, TraceMode=:silent)
				
				Hkg_Inv =10.^ (BlackBoxOptim.best_candidate(Optimization)[1])
				σ_Inv = BlackBoxOptim.best_candidate(Optimization)[2]
				Ks_Inv = 10.^(BlackBoxOptim.best_candidate(Optimization)[3])
			
			else
				Optimization = BlackBoxOptim.bboptimize(X -> OF_BEST(Time[1:iT_N], Inf_3D_Obs[1:iT_N], iT_N, θ_Ini, θs, θr, 10.^X[1], σ, 10.^X[2], RingRadius, iT_TransStead, Time_TransStead, Flag_Best, P_Stead, P_Trans); SearchRange = [(log10(param.Hkg_Min), log10(param.Hkg_Max)), (param.Ks_Log_Min, param.Ks_Log_Max)], NumDimensions=2, TraceMode=:silent)
				Hkg_Inv =10.^ (BlackBoxOptim.best_candidate(Optimization)[1])
				Ks_Inv = 10.^ (BlackBoxOptim.best_candidate(Optimization)[2])
				σ_Inv = σ
			end
    ...
    #= =============== VAN GENUCHTEN =============== =#
	module vg
	using cst, kunsat, param, BlackBoxOptim, sorptivity, wrc, best, stats
	export BEST_INVERSE_SORPTIVITY, BEST_INVERSE_IMPROVED, STEADY_TRANSIT_BESTG
		...
		function BESTG_INVERSE(Time, iT_N, Inf_3D_Obs, θ_Ini, Se_Ini, θs, Km, RingRadius, iT_TransStead, Time_TransStead; Option_Opt_N=true, Flag_Best="Best_Gi", N=1.5)
			...
			if Option_Opt_N
				...
				Optimization = bboptimize(X -> OF_BEST(Time[1:iT_N], Inf_3D_Obs[1:iT_N], iT_N, θ_Ini, θs, θr, 10.^X[1], X[2], 10.^X[3], RingRadius, Km); SearchRange = [(log10(param.Hvg_Min), log10(param.Hvg_Max)), (N_Min, N_Max), (param.Ks_Log_Min, param.Ks_Log_Max)], NumDimensions=3, TraceMode=:silent)

					Hvg_Inv =10.^(best_candidate(Optimization)[1])
					N_Inv = best_candidate(Optimization)[2]
					Ks_Inv = 10. ^(best_candidate(Optimization)[3])
			
				else
				Optimization = bboptimize(X -> OF_BEST(Time[1:iT_N], Inf_3D_Obs[1:iT_N], iT_N, θ_Ini, θs, θr, 10.^X[1], N, 10.^X[2], RingRadius, Km); SearchRange = [(log10(param.Hvg_Min), log10(param.Hvg_Max)), (param.Ks_Log_Min, param.Ks_Log_Max)], NumDimensions=2, TraceMode=:silent)
					Hvg_Inv =10.^(best_candidate(Optimization)[1])
