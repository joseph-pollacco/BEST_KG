...
module diffusivity
	...
	module kg
		...
		# Diffusivity2 = sqrt(Se*π/8.)*Hkg*σ*Ks*((erfc( erfcinv((2.*Se)) + σ/sqrt(2.) ))^2.)/(exp(-(2.+σ*sqrt(2.))*erfcinv(2.*Se)))end
		end
	end
	...
	module vg
		...
		function DIFFUSIVITY_2(θ, θs, θr, Hvg, N, Ks, Km)
			...
			# Diffusivity = ((1.-M)*Ks*Hvg)/(Km*M*(θs-θr)) * (Se.^ (((Km^3)*M+Km-3)/(2*M))) * ( (1.-Se.^(1/M) ).^((1-M-Km)/(Km)) - ((-1)^Km)*(1.-Se.^(1/M)).^((M+1-Km)/(Km)) + 2.*(2.-Km) )

			# D. Moret-Fernández, B. Latorre / Journal of Hydrology 544 (2017) 352–362
			Diffusivity=(((1.-M)*Ks*Hvg)/(M*(θs-θr))) * (Se.^ (0.5-(1./M))) * ( ((1.-Se.^(1./M)).^(-M))  +  ((1.-Se.^(1/M)).^M)  -2. )
